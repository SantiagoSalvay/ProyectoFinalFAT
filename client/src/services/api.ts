...
class ApiService {
  ...
  async adminListComments(
    ...
    return this.request<{ comentarios: any[] }>(
  ...
  async adminUpdateComment(
    ...
    return this.request<{ message: string; comentario: any }>(
  ...
  async adminUpdateUser(
    id: number,
    data: { nombre?: string; apellido?: string; ubicacion?: string },
  ) {
  ...
  async adminBanUser(
    id: number,
    data: { reason?: string; days?: number; permanent?: boolean },
  ) {
  ...
  async adminUpdateONG(
    id: number,
    data: { nombre?: string; ubicacion?: string },
  ) {
    return this.request<{ message: string; ong: any }>(
      `/api/admin/ongs/${id}`,
      {
        method: "PUT",
        body: JSON.stringify(data),
  ...
  async adminListUsersAll(params?: {
    type?: "all" | "user" | "ong";
    q?: string;
  }) {
    ...
    return this.request<{ users: any[] }>(
      `/api/admin/users-all${qs ? `?${qs}` : ""}`,
      { method: "GET" },
    );
  }
  async adminUpdateActor(
    id: number,
    data: { nombre?: string; apellido?: string; ubicacion?: string },
  ) {
    return this.request<{ message: string; actor: any }>(
      `/api/admin/actors/${id}`,
      {
        method: "PUT",
        body: JSON.stringify(data),
  ...
  async adminListPosts(q?: string) {
    ...
    return this.request<{ posts: any[] }>(`/api/admin/posts${qs}`, {
      method: "GET",
    });
  }
  ...
  async adminListDonations(q?: string) {
    const qs = q ? `?q=${encodeURIComponent(q)}` : "";
    return this.request<{ donations: any[] }>(`/api/admin/donations${qs}`, {
      method: "GET",
    });
  }
  async adminUpdateDonation(
    ...
    data: {
      cantidad?: number;
      horas_donadas?: number;
      puntos_otorgados?: number;
    },
  ) {
    return this.request<{ message: string; donation: any }>(
  ...
  async adminFlagDonation(id: number, reason?: string) {
    return this.request<{ message: string; infraction: any }>(
      `/api/admin/donations/${id}/flag`,
      {
        method: "POST",
  ...
  async adminGetLogs(limit = 200) {
    return this.request<{ logs: any[] }>(`/api/admin/logs?limit=${limit}`, {
      method: "GET",
    });
  }
  ...
  async createMPPreference(params: {
    ongId: number;
    description: string;
    amount: number;
    quantity?: number;
  }) {
    return this.request<{ id: string; init_point: string }>(
  ...
  async getTipoONGById(id_usuario: number) {
    try {
      const response = await fetch(
        `${API_BASE_URL}/auth/profile/tipoong/${id_usuario}`,
        {
          method: "GET",
          headers: {
  ...
  async getONGs(filters?: { type?: string; location?: string }) {
    try {
      ...
      console.log(
        "✅ [API] ONGs obtenidas exitosamente:",
        response.ongs?.length || 0,
        "ONGs",
      );
      return response;
    } catch (error) {
  ...
  async register(userData: {
    ...
    try {
      ...
      console.log("Registro exitoso:", {
        ...
        user: response.user
          ? { ...response.user, contrasena: undefined }
          : undefined,
      });
  ...
  async updateProfile(profileData: {
    ...
    try {
      ...
      if (profileData.redes_sociales) {
        console.log(
          "📤 [API] Redes sociales a enviar:",
          profileData.redes_sociales,
        );
      ...
      if (response.user?.redes_sociales) {
        console.log(
          "✅ [API] Redes sociales en respuesta:",
          response.user.redes_sociales,
        );
      }
  ...
  async getCategorias() {
    try {
      const response = await this.request<
        { id_etiqueta: number; etiqueta: string }[]
  ...
  async getPublicaciones() {
    try {
      const response = await this.request<any[]>("/api/forum/publicaciones", {
        method: "GET",
      });
      return response;
    } catch (error) {
  ...
  // Métodos para gestión de imágenes de perfil de ONG
  async uploadONGProfileImage(imageFile: File) {
    try {
      const formData = new FormData();
      formData.append("image", imageFile);

      const token = this.getToken();
      const response = await fetch(`${API_BASE_URL}/api/ongs/profile-image`, {
        method: "POST",
        headers: {
      ...
      const data = await response.json();
      console.log("✅ Imagen de perfil subida:", data);
      return data;
    } catch (error) {
  ...
  async getONGProfileImage(ongId: number) {
    try {
      const response = await this.request<{ imageUrl: string | null }>(
        `/api/ongs/${ongId}/profile-image`,
        {
          method: "GET",
        },
      );
  ...
  async deleteONGProfileImage() {
    try {
      const response = await this.request<{ message: string }>(
        `/api/ongs/profile-image`,
        {
          method: "DELETE",
        },
      );
