...
app.get('/health/db', async (req, res) => {
  try {
    ...
    const result = await prisma.$queryRaw`SELECT to_regclass('"public"."Usuario"') AS usuario, to_regclass('"public"."TipoUsuario"') AS tipousuario`;
    const row = Array.isArray(result) ? result[0] : result;
    const missing = [];
    if (!row?.usuario) missing.push('public.Usuario');
    if (!row?.tipousuario) missing.push('public.TipoUsuario');
    if (missing.length) {
      return res.status(500).json({ ok: false, reason: 'missing_tables', details: { missing } });
    }
    ...
    res.status(500).json({ ok: false, reason: 'db_error', code, message: map[code] || err?.message || 'Error de conexión' });
  }
});
